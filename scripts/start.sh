...
if [ -z "$SERVER_HOST" ] || [ -z "$SERVER_PORT" ]; then
  echo "ERROR: Railway TCP Proxy host/port are unavailable. Create a TCP Proxy targeting application port 8080 before starting REALITY nodes." >&2
  exit 1
fi
case "$SERVER_PORT" in *[!0-9]* ) echo "ERROR: invalid TCP proxy port: $SERVER_PORT" >&2; exit 1;; esac
case "$TCP_APPLICATION_PORT" in *[!0-9]* ) echo "ERROR: invalid TCP application port: $TCP_APPLICATION_PORT" >&2; exit 1;; esac
...
printf '%s\n' "[startup] Railway public domain: $PUBLIC_DOMAIN" "[startup] TCP proxy: $SERVER_HOST:$SERVER_PORT" "[startup] TCP application port: $TCP_APPLICATION_PORT" "[startup] Gateway: 0.0.0.0:$GATEWAY_PORT" "[startup] Xray REALITY: $XRAY_LISTEN:$XRAY_PORT" "[startup] Xray HTTPS/XHTTP: $XRAY_LISTEN:$XRAY_HTTP_PORT"
python3 /opt/xray/scripts/generate.py
printf '%s/sub/%s\n' "${PUBLIC_SUBSCRIPTION_URL%/}" "$SUBSCRIPTION_TOKEN" > "$URL_FILE"
printf '%s\n' "$(cat "$DATA_DIR/subscription_fallback_url.txt")" > "$DATA_DIR/subscription_fallback_url.txt"
chmod 0600 "$URL_FILE" "$DATA_DIR/subscription_fallback_url.txt"
