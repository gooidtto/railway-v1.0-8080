...
def _proxy_header_length(data):
    if PROXY_PROTOCOL == "off": return 0
...
def _strip_proxy_header(data):
    n = _proxy_header_length(data)
    if n is None: return data, True
    return (data[n:], False) if n else (data, False)
...
def recv_initial(s, timeout=10):
    ...
    while len(data) < 16384:
        ...
        if not proxy_header_seen:
            ...
            if pending: continue
            proxy_header_seen = True; data = bytearray(raw)
        if not raw: continue
        if is_tls(raw): return raw, "tls"
        if b"\r\n\r\n" in raw or b"\n\n" in raw: return raw, "http"
        if len(raw) >= 3 and not raw.startswith(methods): return raw, "tcp"
    raw = bytes(data)
    if not proxy_header_seen: raw, _ = _strip_proxy_header(raw)
    if not raw: return b"", "empty"
    if is_tls(raw): return raw, "tls"
...
def handle_http(c, method, path):
    ...
    if path == "/ready":
        ok = ready(); c.sendall(response(200 if ok else 503, "text/plain; charset=utf-8", "READY\n" if ok else "NOT READY\n", head)); return True
...
def handle(c, allow_tls=True, listener="gateway"):
    ...
    try:
        p = c.getpeername(); peer = f"{p[0]}:{p[1]}"
    except OSError: pass
    tune(c)
    ...
    try:
        ...
        if not initial: log(f"CLOSE listener={listener} peer={peer} reason=no-initial-data"); return
        log(f"CLASSIFY listener={listener} peer={peer} kind={kind} bytes={len(initial)} head={initial[:12].hex()}")
        if kind == "tls":
            if not allow_tls: return
            ...
            a,b=relay(c,up,initial); log(f"RELAY_END listener={listener} peer={peer} kind=tls-reality c2s={a} s2c={b}"); return
        parsed = parse_http(initial)
        if parsed:
            method,path=parsed; log(f"HTTP listener={listener} peer={peer} method={method} path={path}")
            if handle_http(c,method,path): log(f"HTTP_END listener={listener} peer={peer} path={path}"); return
            kind="http-xhttp"
        target=(HTTP_XHTTP_HOST,HTTP_XHTTP_PORT) if kind=="http-xhttp" else (REALITY_HOST,REALITY_PORT)
        up=connect(*target); log(f"UPSTREAM_CONNECTED listener={listener} peer={peer} target={target[0]}:{target[1]} kind={kind}")
        a,b=relay(c,up,initial); log(f"RELAY_END listener={listener} peer={peer} kind={kind} c2s={a} s2c={b}")
    except (OSError,TimeoutError) as e: log(f"ERROR listener={listener} peer={peer} type={type(e).__name__} detail={e}")
    finally:
        if up:
            try: up.close()
            except OSError: pass
...
def serve(port, allow_tls, listener):
    with socket.socket(socket.AF_INET,socket.SOCK_STREAM) as s:
        ...
        while True:
            c,_=s.accept(); threading.Thread(target=handle,args=(c,allow_tls,listener),daemon=True).start()


def main():
    if HTTP_LISTEN_PORT == GATEWAY_LISTEN_PORT: serve(GATEWAY_LISTEN_PORT,True,"gateway"); return
    threads=[threading.Thread(target=serve,args=(HTTP_LISTEN_PORT,False,"http"),daemon=True),threading.Thread(target=serve,args=(GATEWAY_LISTEN_PORT,True,"gateway"),daemon=True)]
