...
def normalize_target(value):
    ...
    if value.startswith(("http://", "https://")):
        parsed = urlparse(value); value = parsed.netloc or parsed.path
...
for raw in sni_file.read_text(encoding="utf-8").splitlines():
    value = raw.strip()
    if value and not value.startswith("#") and value not in pool: pool.append(value)
...
reality_inbound = {
    ...
    "streamSettings": {
        ...
        "realitySettings": {"show": False, "target": target, "xver": 0, "serverNames": pool, "privateKey": private_key, "shortIds": [short_id]},
