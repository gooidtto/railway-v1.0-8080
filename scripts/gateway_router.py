...
def tls_sni(data):
    # Minimal TLS ClientHello parser. TLS is never terminated here.
    try:
        ...
        while p + 4 <= end:
            ...
            ln = int.from_bytes(data[p+2:p+4], 'big'); p += 4
            if p + ln > end: break
            if typ == 0:
                block = data[p:p+ln]
                if len(block) < 5: return ''
...
def recv_initial(s, timeout=10):
    ...
    while len(data) < 32768:
        chunk = s.recv(min(4096, 32768 - len(data)))
        if not chunk: break
        data.extend(chunk)
        raw = bytes(data)
        if b'\r\n\r\n' in raw:
        ...
        if len(raw) >= 5 and raw[0] == 0x16 and raw[1] == 0x03:
            record_len = int.from_bytes(raw[3:5], 'big')
...
def handle(c):
    ...
    try:
        ...
        # Opaque TCP is reserved for the high-throughput Vision listener.
