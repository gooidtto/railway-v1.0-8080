...
def candidate_list(name, fallback):
    ...
    values = []
    if path.is_file():
        values = [x.strip() for x in path.read_text(encoding='utf-8').splitlines()
...
def inbound(port, network, security, uuid, decryption, reality=None, xhttp=None, grpc=None, flow=None):
    ...
    return {'listen': '127.0.0.1', 'port': port, 'protocol': 'vless',
...
def main():
    ...
    all_candidates = candidate_list('REALITY_SNI_CANDIDATES_FILE', ['www.cloudflare.com'])
    xhttp_names = candidate_list('XHTTP_REALITY_SNI_FILE', all_candidates[:max(1, min(2, len(all_candidates)))])
    vision_names = candidate_list('VISION_REALITY_SNI_FILE', all_candidates[2:4] or all_candidates[:1])
    grpc_names = candidate_list('GRPC_REALITY_SNI_FILE', all_candidates[4:6] or all_candidates[:1])

    ports = runtime['listeners']
    inbounds = [
        inbound(ports['xhttp_reality'], 'xhttp', 'reality', uuid, decryption,
                reality=reality_settings(target, xhttp_names, private, sid),
        ...
        inbound(ports['vision_reality'], 'raw', 'reality', uuid, decryption,
                reality=reality_settings(target, vision_names, private, sid),
                flow='xtls-rprx-vision'),
        inbound(ports['grpc_reality'], 'grpc', 'reality', uuid, decryption,
                reality=reality_settings(target, grpc_names, private, sid),
